...
fMeas = S11_4mm.Frequencies;
% NfpointsMeas = length(fMeas);
% S11_4mm = squeeze(S11_4mm.Parameters);
% S11_16mm = squeeze(S11_16mm.Parameters);
% S11_28mm = squeeze(S11_28mm.Parameters);
...
function Z_in = impedanceTLM(epsR, h, W, L, L1, f)
    lambda0 = c./f;
    k0 = 2*pi./lambda0;
    epsR_eff = (epsR+1)/2+(epsR-1)/2*(1+12*h./W).^(-1/2);
    ...
    beta = 2*pi*sqrt(epsR_eff)/lambda0;
    L2 = L - L1;
    Z1 = Z_c.*(Z_s+1i*Z_c.*tan(beta*L1))./(Z_c+1i.*Z_s.*tan(beta*L1));
    Z2 = Z_c.*(Z_s+1i*Z_c.*tan(beta*L2))./(Z_c+1i.*Z_s.*tan(beta*L2));
    Z_in = 1./(1./Z1+1./Z2);
end