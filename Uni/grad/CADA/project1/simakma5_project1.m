...
for alpha = alpha_range                         % calculate everything for different alphas
    ...
    while Rmax > 1e-3                           % relaxation until improvement is insignificant
        ...
        % Step 1: SOR method
        for j = 2:Nx-1
            for min_index = 2:Ny-1
                if min_index == IF1             % nodes on interface 1
                    R(min_index,j)= 1/4*(eps_r*V(min_index+1,j)+1*V(min_index-1,j)+er_avg*V(min_index,j-1)+er_avg*V(min_index,j+1))/er_avg-V(min_index,j); 
                elseif min_index == IF2         % nodes on interface 2
                    R(min_index,j)= 1/4*(1*V(min_index+1,j)+eps_r*V(min_index-1,j)+er_avg*V(min_index,j-1)+er_avg*V(min_index,j+1))/er_avg-V(min_index,j); 
                else                            % free nodes
                    R(min_index,j)= 1/4*(V(min_index+1,j)+V(min_index-1,j)+V(min_index,j-1)+V(min_index,j+1))-V(min_index,j);
                end
                    V(min_index,j)= V(min_index,j)+alpha*R(min_index,j);
        ...
        qA = 0; qB = 0; qC = 0; qD = 0;
        for min_index=top:1:bottom
            if min_index < IF1
                qA = qA + eps_r*(V(min_index,left+1) - V(min_index,left-1))/2;
                qC = qC + eps_r*(V(min_index,right-1) - V(min_index,right+1))/2;
            elseif min_index == IF1
                qA = qA + er_avg*(V(min_index,left+1) - V(min_index,left-1))/2;
                qC = qC + er_avg*(V(min_index,right-1) - V(min_index,right+1))/2;
            else
                qA = qA + (V(min_index,left+1) - V(min_index,left-1))/2;
                qC = qC + (V(min_index,right-1) - V(min_index,right+1))/2;
            end
        end
        for j=left:1:right
...
% Evolution of the line impedance with the number of iterations
figure(2)
plot(Z)
axis tight
ylabel('Z_0 [\Omega]')
...
% Compare numerically optimal value of alpha with an analytical solution
